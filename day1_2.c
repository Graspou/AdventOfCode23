...
char firstDigit(const char *str) { 
    ...
    while (*(str + i) != '\n' && *(str + i)) { 
        if (*(str + i) >= '0' && *(str + i) <= '9') 
            return *(str + i);
        char *tmp3 = malloc(3 * sizeof(char));
        char *tmp4 = malloc(3 * sizeof(char));
        char *tmp5 = malloc(3 * sizeof(char));
        slice(str, tmp3, i, i+3);
        slice(str, tmp4, i, i+4);
        slice(str, tmp5, i, i+5);
        char *arr[9] = {"one", "two", "three", "four", "five", "six", "seven", "eight", "nine"};
        for (size_t j = 0; j<9; ++j) { 
            if (strcmp(tmp3, arr[j]) == 0 || strcmp(tmp4, arr[j]) == 0 || strcmp(tmp5, arr[j]) == 0) {
                free(tmp3);
                free(tmp4);
                free(tmp5);
                return '0' + j+1;
...
int main(int argc, char **argv) { 
    printf("%c \n", firstDigit("avec three plaisir 3 \n"));
